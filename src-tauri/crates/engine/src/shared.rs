...
fn shooting_profile(snap: &PlayerSnap) -> f64 {
    normalize_strength(
        weighted_mean(&[
            (snap.shooting, 0.45),
            (snap.composure, 0.25),
...
fn calmness_profile(snap: &PlayerSnap) -> f64 {
    normalize_strength(
        weighted_mean(&[
            (snap.composure, 0.55),
            (snap.decisions, 0.30),
...
fn forward_package_profile(snap: &PlayerSnap) -> f64 {
    normalize_strength(
        weighted_mean(&[
            (snap.shooting, 0.25),
            (snap.dribbling, 0.15),
...
fn carry_profile(snap: &PlayerSnap) -> f64 {
    normalize_strength(
        weighted_mean(&[
            (snap.dribbling, 0.45),
            (snap.agility, 0.30),
...
fn playmaking_profile(snap: &PlayerSnap) -> f64 {
    normalize_strength(
        weighted_mean(&[
            (snap.passing, 0.35),
            (snap.vision, 0.30),
...
fn set_piece_profile(snap: &PlayerSnap) -> f64 {
    normalize_strength(
        weighted_mean(&[
            (snap.passing, 0.35),
            (snap.shooting, 0.20),
...
fn ball_winning_profile(snap: &PlayerSnap) -> f64 {
    normalize_strength(
        weighted_mean(&[
            (snap.tackling, 0.35),
            (snap.defending, 0.25),
            (snap.aggression, 0.20),
            (snap.stamina, 0.20),
        ]),
        62.0,
        92.0,
...
fn defensive_shape_profile(snap: &PlayerSnap) -> f64 {
    normalize_strength(
        weighted_mean(&[
            (snap.defending, 0.35),
            (snap.positioning, 0.30),
            (snap.strength, 0.20),
            (snap.tackling, 0.15),
        ]),
        62.0,
...
fn aerial_power_profile(snap: &PlayerSnap) -> f64 {
    normalize_strength(
        weighted_mean(&[
            (snap.aerial, 0.40),
            (snap.strength, 0.35),
            (snap.aggression, 0.10),
            (snap.positioning, 0.15),
        ]),
        62.0,
        92.0,
...
fn goalkeeping_handling_profile(snap: &PlayerSnap) -> f64 {
    normalize_strength(
        weighted_mean(&[
            (snap.handling, 0.50),
            (snap.reflexes, 0.20),
...
fn goalkeeping_reflex_profile(snap: &PlayerSnap) -> f64 {
    normalize_strength(
        weighted_mean(&[
            (snap.reflexes, 0.55),
            (snap.agility, 0.15),
...
fn work_rate_profile(snap: &PlayerSnap) -> f64 {
    normalize_strength(
        weighted_mean(&[
            (snap.stamina, 0.40),
            (snap.teamwork, 0.25),
...
fn aggression_profile(snap: &PlayerSnap) -> f64 {
    normalize_strength(
        weighted_mean(&[
            (snap.aggression, 0.60),
            (snap.tackling, 0.15),
            (snap.strength, 0.15),
            (snap.decisions, 0.10),
        ]),
...
pub(crate) fn trait_bonus(snap: &PlayerSnap, context: TraitContext) -> f64 {
    ...
    match context {
        TraitContext::Shooting => {
            bonus *= 1.0 + shooting_profile(snap) * 0.08;
            bonus *= 1.0 + calmness_profile(snap) * 0.04;
            bonus *= 1.0 + forward_package_profile(snap) * 0.05;
        ...
        TraitContext::Dribbling => {
            bonus *= 1.0 + carry_profile(snap) * 0.11;
        ...
        TraitContext::Passing => {
            bonus *= 1.0 + playmaking_profile(snap) * 0.10;
            bonus *= 1.0 + set_piece_profile(snap) * 0.03;
        ...
        TraitContext::Tackling => {
            bonus *= 1.0 + ball_winning_profile(snap) * 0.08;
            bonus *= 1.0 + defensive_shape_profile(snap) * 0.05;
            bonus *= 1.0 + aerial_power_profile(snap) * 0.03;
        ...
        TraitContext::Goalkeeping => {
            bonus *= 1.0 + goalkeeping_handling_profile(snap) * 0.08;
            bonus *= 1.0 + goalkeeping_reflex_profile(snap) * 0.06;
            bonus *= 1.0 + aerial_power_profile(snap) * 0.04;
        ...
        TraitContext::Foul => {
            bonus *= 1.0 + aggression_profile(snap) * 0.20;
            bonus *= 1.0 - calmness_profile(snap) * 0.16;
        ...
        TraitContext::Midfield => {
            bonus *= 1.0 + work_rate_profile(snap) * 0.08;
            bonus *= 1.0 + playmaking_profile(snap) * 0.03;
...
pub(crate) fn choose_defensive_intent<R: Rng>(
    ...
    let high_press_fit = if matches!(style, PlayStyle::HighPress) {
        work_rate_profile(snap) * 0.42
            + ball_winning_profile(snap) * 0.32
            + trait_strength(snap, "TransitionAnticipator") * 0.24
            + playmaking_profile(snap) * 0.08
    } else {
        0.0
    };
    let weights = vec![
    ...
    let weights = vec![
        (ActionIntent::SecureClaim, {
            0.78 + context.safe_outlets * 0.34
                ...
                + goalkeeping_handling_profile(snap) * 0.20
                + if context.protecting_lead { 0.18 } else { 0.0 }
        }),
...
pub(crate) fn trait_involvement_modifier(
    ...
    let bonus = match preferred {
        Position::Goalkeeper => {
            goalkeeping_handling_profile(&snap) * 0.09
                + goalkeeping_reflex_profile(&snap) * 0.08
                + aerial_power_profile(&snap) * 0.04
                + sum_trait_weights(
                    &snap,
                    &[
                        ("LineDictator", 0.04),
        ...
        Position::Defender => {
            defensive_shape_profile(&snap) * 0.08
                + ball_winning_profile(&snap) * 0.05
                + sum_trait_weights(
                    &snap,
                    &[
        ...
        Position::Midfielder => {
            playmaking_profile(&snap) * 0.07
                + work_rate_profile(&snap) * 0.05
                + sum_trait_weights(
                    &snap,
                    &[
                        ("EarlyScanner", 0.04),
        ...
        Position::Forward => {
            shooting_profile(&snap) * 0.07
                + forward_package_profile(&snap) * 0.05
                + sum_trait_weights(
                    &snap,
                    &[
...
pub(crate) fn trait_pass_bias(snap: &PlayerSnap) -> f64 {
    ...
    ) + playmaking_profile(snap) * 0.12
        + set_piece_profile(snap) * 0.03)
        .clamp(0.88, 1.22)
}
...
pub(crate) fn trait_shot_bias(snap: &PlayerSnap) -> f64 {
    ...
    ) + shooting_profile(snap) * 0.11
        + forward_package_profile(snap) * 0.04)
        .clamp(0.84, 1.24)
}
...
pub(crate) fn trait_clearance_bias(snap: &PlayerSnap) -> f64 {
    ...
    ) - sum_trait_weights(snap, &[("ThrowLauncher", 0.02)])
        + defensive_shape_profile(snap) * 0.07
        + aerial_power_profile(snap) * 0.04
        - playmaking_profile(snap) * 0.05)
        .clamp(0.84, 1.20)
}
...
pub(crate) fn trait_press_bias(snap: &PlayerSnap) -> f64 {
    ...
    ) + ball_winning_profile(snap) * 0.08
        + work_rate_profile(snap) * 0.05
