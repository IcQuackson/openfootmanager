...
use crate::shared::{
    ActionIntent, IntentContext, TraitContext, attack_modifier, box_entry_chance, buildup_modifier,
    ...
    formation_midfield_modifier, formation_rest_defense_modifier, home_mod,
    midfield_attack_modifier, midfield_defense_modifier, press_modifier, role_carry_bias,
    role_clearance_bias, role_pass_bias, role_press_bias, role_shot_bias, trait_bonus,
    trait_carry_bias, trait_clearance_bias, trait_pass_bias, trait_press_bias, trait_shot_bias,
    trait_strength, transition_entry_chance,
};
...
fn resolve_buildup<R: Rng>(
    ...
    let intent =
        choose_buildup_intent(&passer, ctx.team(att_side).play_style, buildup_context, rng);
    let opponent_rest_defense = formation_rest_defense_modifier(&ctx.team(def_side).formation)
        * defense_modifier(ctx.team(def_side).play_style);
    let progression_jump = transition_entry_chance(
        ctx.team(att_side).play_style,
        &ctx.team(att_side).formation,
        passer.role,
        buildup_context,
    ...
    let (intent_modifier, success_zone, keeps_transition) = match intent {
        ...
        ActionIntent::SplitLinePass => (
            0.95,
            if buildup_context.in_transition || rng.gen_range(0.0..1.0f64) < progression_jump {
                Zone::attacking_third(att_side)
            } else {
                Zone::Midfield
            },
            buildup_context.in_transition,
        ),
        ActionIntent::BaitPressTouch => (
            0.90,
            if buildup_context.under_pressure
                || buildup_context.in_transition
    ...
    if rng.gen_range(0.0..1.0f64) < success_chance {
        ...
        let recovery_context = intent_context(ctx, minute, def_side, false);
        let breakout = transition_entry_chance(
            ctx.team(def_side).play_style,
            &ctx.team(def_side).formation,
            presser.role,
            recovery_context,
            formation_rest_defense_modifier(&ctx.team(att_side).formation)
                * defense_modifier(ctx.team(att_side).play_style),
        );
        ctx.ball_zone = if rng.gen_range(0.0..1.0f64) < breakout {
...
fn handle_attacking_third_defensive_win<R: Rng>(
    ...
    if rng.gen_range(0.0..1.0f64)
        < if attacker_intent == ActionIntent::LateBoxDelivery {
            0.22 + base_corner_pressure(ctx, att_side)
        } else {
            0.16 + base_corner_pressure(ctx, att_side)
        }
    {
        ctx.emit(MatchEvent::new(minute, EventType::Corner, att_side, zone));
    ...
    ctx.ball_zone = if rng.gen_range(0.0..1.0f64) < breakout * 0.85 {
        Zone::Midfield
    } else {
...
fn maybe_rebound_follow_up<R: Rng>(
    ...
    if rng.gen_range(0.0..1.0f64) < (0.14 + (finish_rating - gk_rating) / 260.0).clamp(0.05, 0.38) {
        ctx.emit(
...
fn settle_after_goalkeeper_control<R: Rng>(
    ...
    let distribution_intent = choose_goalkeeper_distribution_intent(
        ...
        enrich_intent_context(
            ...
            ctx.team(possession_side).play_style,
            &ctx.team(possession_side).formation,
        ),
        rng,
    );
            ...
            ctx.ball_zone = if rng.gen_range(0.0..1.0f64)
                < transition_entry_chance(
                    ...
                    formation_rest_defense_modifier(
                        &ctx.team(possession_side.opposite()).formation,
                    ) * defense_modifier(ctx.team(possession_side.opposite()).play_style),
                ) {
...
fn intent_context(
    ...
    enrich_intent_context(
        ...
        ctx.team(side).play_style,
        &ctx.team(side).formation,
    )
}

fn base_corner_pressure(ctx: &MatchContext, side: Side) -> f64 {
    (formation_attack_modifier(&ctx.team(side).formation)
        + attack_modifier(ctx.team(side).play_style, ctx.transition_side == Some(side)))
        * 0.05
}
