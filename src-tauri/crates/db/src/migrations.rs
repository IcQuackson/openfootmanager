...
pub fn all_migrations() -> Migrations<'static> {
    Migrations::new(vec![
        ...
        // V7: Persist per-player per-match stat history
        M::up(include_str!("sql/v007_player_match_stats.sql")),
        // V8: Team financial ledger
        M::up(include_str!("sql/v007_team_financial_ledger.sql")),
        // V9: Team sponsorship state
        M::up(include_str!("sql/v008_team_sponsorship.sql")),
        // V10: Team facilities state
        M::up(include_str!("sql/v009_team_facilities.sql")),
        // V11: Hidden per-player morale architecture state
        M::up(include_str!("sql/v010_player_morale_core.sql")),
        // V12: Player footedness identity fields
        M::up(include_str!("sql/v011_player_footedness.sql")),
        // V13: Fixture competition metadata
        M::up(include_str!("sql/v012_fixture_competition.sql")),
        // V14: Player long-term fitness value
        M::up(include_str!("sql/v013_player_fitness.sql")),
        // V15: Persistent team tactical role slots
        M::up(include_str!("sql/v014_team_tactical_roles.sql")),
        // V16: Split player profile badges from engine traits
        M::up(include_str!("sql/v015_player_badges_engine_traits.sql")),
        // V17: Persist per-player development plans (trait + position learning)
        M::up(include_str!("sql/v016_player_development_plan.sql")),
    ])
}
